...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (medias.length > 0) {
      posthogClient.capture({
        distinctId: userId,
        event: `ext-books-insert uploading media`,
        properties: {
    ...
    posthogClient.capture({
      distinctId: userId,
      event: `ext-books-insert uploading sources`,
      properties: {
    ...
    posthogClient.capture({
      distinctId: userId,
      event: `ext-books-insert finished`,
      properties: {
