...
export async function POST(request: NextRequest) {
  ...
  const matchesUserId = bearerValue.split("~~~")[1];
  ...
  posthogClient.capture({
    distinctId: userId,
    event: `books-insert-koreader BEGIN`,
  });
  ...
  try {
    ...
    if (medias.length > 0) {
      posthogClient.capture({
        distinctId: userId,
        event: `books-insert-koreader uploading media`,
        properties: {
    ...
    posthogClient.capture({
      distinctId: userId,
      event: `books-insert-koreader uploading sources`,
      properties: {
    ...
    posthogClient.capture({
      distinctId: userId,
      event: `books-insert-koreader finished`,
      properties: {
    ...
    posthogClient.capture({
      distinctId: userId,
      event: `books-insert-koreader ERROR`,
      properties: {
        message:
