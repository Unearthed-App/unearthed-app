...
import { db } from "@/db";
import { notionSourceJobsOne } from "@/db/schema";
...
export async function GET() {
  ...
  const dbResult = await db.query.notionSourceJobsOne.findMany({
    where: eq(notionSourceJobsOne.status, "PENDING"),
    with: {
      source: {
        with: {
          quotes: true,
