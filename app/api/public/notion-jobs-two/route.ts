...
import { db } from "@/db";
import { notionSourceJobsTwo } from "@/db/schema";
...
export async function GET() {
  ...
  const dbResult = await db.query.notionSourceJobsTwo.findMany({
    where: eq(notionSourceJobsTwo.status, "PENDING"),
    with: {
      source: {
        with: {
          quotes: true,
