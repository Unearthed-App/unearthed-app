...
import { db } from "@/db";
import { notionSourceJobsFour } from "@/db/schema";
...
export async function GET() {
  ...
  const dbResult = await db.query.notionSourceJobsFour.findMany({
    where: eq(notionSourceJobsFour.status, "PENDING"),
    with: {
      source: {
        with: {
          quotes: true,
