...
export async function GET() {
  ...
  const notionSourceJobs = await db.query.notionSourceJobsOne.findMany({
    where: and(
      eq(notionSourceJobsOne.status, "READY"),
      or(
        isNull(notionSourceJobsOne.attempts),
        lte(notionSourceJobsOne.attempts, 3)
      )
    ),
    with: {
      source: {
  ...
  await db
    .update(notionSourceJobsOne)
    .set({
      attempts: sql`COALESCE(${notionSourceJobsOne.attempts}, 0) + 1`,
    })
