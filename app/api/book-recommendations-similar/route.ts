...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const systemMessage = `You are a book recommendation expert. Based on the provided book information, suggest similar books that the reader might enjoy. Consider the themes, writing style, and subject matter. Pay special attention to the notes taken.
...
Please provide recommendations for NEW books that are NOT in the user's existing collection. Provide recommendations in this JSON format, using plain text only (no markdown, no special characters, no em dashes):
{
  "recommendations": [
    {
      "title": "Book Title",
      "author": "Author Name",
      "reason": "Brief explanation of why this book is recommended (use regular quotes, no italics or special formatting)"
    }
  ]
}`;
    ...
    posthogClient.capture({
      distinctId: userId,
      event: "book-recommendations complete",
      properties: {
        inputTokens,
        outputTokens,
